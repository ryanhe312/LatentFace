...
class EDDeconv(nn.Module):
    ...
    def forward(self, input, expr_code = 0, weight = 1, mean = True):
        code = self.network1(input).squeeze(-1).squeeze(-1)
        new_code = code.mean(dim=0).expand_as(code) if mean else code
        new_code = new_code + expr_code * weight
        new_code = new_code.unsqueeze(-1).unsqueeze(-1)
        return code, self.network2(new_code)