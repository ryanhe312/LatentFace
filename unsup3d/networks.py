...
class Encoder(nn.Module):
    def __init__(self, cin, cout, nf=64, activation=nn.Tanh, repeat=0):
...
class EDDeconv(nn.Module):
    def __init__(self, cin, cout, zdim=128, nf=64, activation=nn.Tanh, repeat=0):
        ...
        network1 += [
            nn.Conv2d(nf*8, nf*8, kernel_size=4, stride=2, padding=1, bias=False),  # 8x8 -> 4x4
            nn.LeakyReLU(0.2, inplace=True)] * repeat
        network1 += [
        ...
        network2 = [
            ...
            nn.ReLU(inplace=True)]
        network2 += [
            nn.ConvTranspose2d(nf*4, nf*4, kernel_size=4, stride=2, padding=1, bias=False),  # 4x4 -> 8x8
            nn.GroupNorm(16*4, nf*4),
            nn.ReLU(inplace=True),
            nn.Conv2d(nf*4, nf*4, kernel_size=3, stride=1, padding=1, bias=False),
            nn.GroupNorm(16*4, nf*4),
    ...
    def forward(self, input):
        code = self.network1(input)
        return code, self.network2(code)


class ConfNet(nn.Module):
    def __init__(self, cin, cout, zdim=128, nf=64, repeat=0):
