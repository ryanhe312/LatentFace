...
from . import meters
from . import utils
from .dataloaders_video import get_data_loaders
...
class Trainer_video():
    ...
    def train(self):
        ...
        if self.resume:
            ...
            # self.save_clean_checkpoint(os.path.join(self.checkpoint_dir, self.checkpoint_name+'.clean'))
        ...
        print(f"Training completed after {self.num_epochs+1} epochs.")
    ...
    def run_epoch(self, loader, epoch=0, is_validation=False, is_test=False):
        ...
        for iter, input in enumerate(loader):
            ...
            est_time = int((time.time()-st)/(iter+1)*(iters-(iter+1)))
            print(f"{'T' if is_train else 'V'}{epoch:02}/{iter:05}/{iters:05}/{est_time//60//60}h{est_time//60%60}m\t {metrics}")
